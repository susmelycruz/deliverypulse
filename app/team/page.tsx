    ...
    <AppShell>
      <div className="card">
        ...
        <form action={addMember} className="grid two">
          ...
          <div style={{alignSelf:'end'}}><label><input type="checkbox" name="is_admin" /> Admin access</label></div>
          <div style={{ alignSelf: 'end' }}><button type="submit">Create member</button></div>
        </form>
      </div>
      <div className="card">
        <h2>Members</h2>
        <table className="table"><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Country</th><th>Active</th><th>Admin</th></tr></thead>
          <tbody>{members?.map((m) => <tr key={m.id}><td>{m.name}</td><td>{m.email}</td><td>{m.role}</td><td>{m.country}</td><td>{String(m.active)}</td><td>{String(m.is_admin)}</td></tr>)}</tbody>
        </table>
      </div>
    </AppShell>
  );
