...
as $$
  select exists (
    select 1 from team_members tm where tm.id = auth.uid() and tm.is_admin = true
  );
$$;
...
create policy "admin_can_manage_team" on team_members
...
create policy "admin_manage_increments" on increments
for all to authenticated
using (is_admin_user())
with check (is_admin_user());
...
create policy "admin_manage_sprints" on sprints
for all to authenticated
using (is_admin_user())
with check (is_admin_user());
...
for select to authenticated using (auth.uid() = member_id or is_admin_user());

create policy "users_insert_own_pto" on pto_entries
for insert to authenticated with check (auth.uid() = member_id or is_admin_user());

create policy "users_update_own_pto" on pto_entries
for update to authenticated
using (auth.uid() = member_id or is_admin_user())
with check (auth.uid() = member_id or is_admin_user());
